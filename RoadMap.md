...
*   **PBI-1.2:** [进行中] 实现词法分析器 (Lexer)
    *   **任务 1.2.1 (TDD):** [进行中] 编写单元测试，覆盖所有关键字 (e.g., `if`, `else`, `while`, `fn`) 的识别。
    *   **任务 1.2.2 (TDD):** [进行中] 编写单元测试，覆盖标识符 (identifiers) 的识别。
    *   **任务 1.2.3 (TDD):** [进行中] 编写单元测试，覆盖数字字面量 (integer and float literals) 的识别。
    *   **任务 1.2.4 (TDD):** [进行中] 编写单元测试，覆盖操作符 (e.g., `+`, `-`, `*`, `/`, `=`, `==`) 的识别。
    *   **任务 1.2.5 (TDD):** [进行中] 编写单元测试，覆盖分隔符 (e.g., `(`, `)`, `{`, `}`, `,`, `;`) 的识别。
    *   **任务 1.2.6:** [进行中] 实现词法分析器逻辑，使所有测试通过。
*   **PBI-1.3:** [未开始] 持续集成 (CI)

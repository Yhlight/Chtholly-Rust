...
*   **任务：**
    *   [ ] **项目设置：**
        ...
        *   [ ] 添加 LLVM 17 绑定（`inkwell` 或 `llvm-sys`）。
        *   [ ] 添加一个解析库（`pest` 或 `lalrpop`）。
        *   [ ] 设置基本的项目结构（例如 `src/main.rs`、`src/lexer.rs`、`src/parser.rs`）。
        *   [ ] 创建一个 `.gitignore` 文件。
    *   [ ] **编译器驱动程序：**
        *   [ ] 实现一个命令行界面来读取 `.cns` 文件。
    *   [ ] **词法分析器（Tokenizer）：**
        *   [ ] 实现注释（单行和多行）的词法分析。
        *   [ ] 实现关键字（`fn`、`let`、`mut` 等）的词法分析。
        *   [ ] 实现标识符、字面量（整数、字符串）和运算符的词法分析。
    *   [ ] **解析器（AST）：**
        *   [ ] 为基本结构定义抽象语法树（AST）节点。
        *   [ ] 实现 `main` 函数的解析。
        *   [ ] 实现变量声明（`let` 和 `mut`）的解析。
    *   [ ] **代码生成（LLVM IR）：**
        *   [ ] 为一个简单的“Hello, World!”程序生成 LLVM IR。
        *   [ ] 编译并运行第一个 Chtholly 程序。

...
- [ ] **Project Setup:** Initialize the Rust project using Cargo.
- [ ] **Roadmap:** Create this `RoadMap.md` file.
- [ ] **Lexer (Tokenizer):** Implement a lexer to convert source code into a stream of tokens.
- [ ] **AST (Abstract Syntax Tree):** Define the data structures for the AST.
- [ ] **Parser:** Implement a parser to build the AST from the token stream.

## Milestone 2: Basic Language Features

- [ ] **Variable Declarations:** Support for `let` and `mut`.
...
- [ ] **Control Flow:** Implement `if-else` and `while` loops.
